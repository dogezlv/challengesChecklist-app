...
`app/overlay/` is a public, session-less page meant to be loaded as an **OBS Browser Source**. `page.tsx` (server) parses query params and renders `Overlay.tsx` (client), which subscribes to Supabase Realtime (`postgres_changes` UPDATE/INSERT on `challenges`) and shows a Fortnite-Season-8-style "challenge completed" notification using the `battle_star`/`battle_pass` icons. Completions **stack**: newest at full size on top, older ones scaled down and dimmed (depth-based scale + CSS transition); each auto-dismisses after `duration`. It detects the `false→true` transition by comparing against a `seen` map seeded on load (so old progress and INSERTs don't fire). Timers for enter/leave/remove are scheduled once in `addNotice` (a ref holds them for cleanup) — do not move that scheduling into a render-driven effect or the dismiss timers cancel themselves. Works for anonymous visitors because anon RLS already allows SELECT + Realtime on `challenges`. The sound is `public/sounds/challenge-complete.mp3`. Params: `?season=<code>` (scope to one season's weeks), `?duration=<ms>` (default 6000), `?sound=0` (mute), `?test=1` (staggered demo notifications for positioning in OBS). Font is the app's (`var(--font-geist-sans)`/Arial) — the user rejected Burbank, so don't reintroduce it here.
