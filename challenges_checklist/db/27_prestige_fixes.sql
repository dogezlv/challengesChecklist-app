-- ============================================================
-- 27_prestige_fixes.sql
--   * S4: daño mira/silenciada → 2 reglas con tag (como kill), OR, misma partida
--   * S9: cabeza de tomate Metrópoli Mercantil (no Durr Burger)
--   * S10: cañón → 7 reglas (campamento destino), origen con nombre
--   * report_event: value + same_match + rules OR suma daño entre reglas
-- ============================================================

-- ── Ubicación: cabeza de tomate Metrópoli Mercantil ─────────────────────────
update public.locations
set code = 'tomato_head_retail',
    display_name = 'Cabeza de tomate (Metrópoli Mercantil)',
    display_name_en = 'Tomato Head (Retail Row)'
where code = 'tomato_head_durr';

-- Si no existía la fila antigua, crear la correcta
insert into public.locations (code, display_name, display_name_en, named_location)
values ('tomato_head_retail', 'Cabeza de tomate (Metrópoli Mercantil)', 'Tomato Head (Retail Row)', false)
on conflict (code) do nothing;

update public.challenge_rules cr
set location_id = (select id from locations where code = 'tomato_head_retail')
where location_id = (select id from locations where code = 'tomato_head_durr');

delete from public.locations where code = 'tomato_head_durr';

-- ── S4: daño con mira o silenciada (2 reglas OR, no condición) ───────────────
do $$
...
begin
  ...
  where s.code = 'season_8' and w.week_number = 4 and c.is_prestige
    and c.description ilike '%da%o con armas con mira o con armas silenciadas%'
  limit 1;
  ...
  update challenges set
    kind = 'progress', unit = 'value', match_scope = 'same_match',
    target_value = 500, current_value = 0, is_completed = false,
    rules_operator = 'or'
  where id = v;

  insert into challenge_rules (challenge_id, action_type_id, required_tag_id)
  select v, (select id from action_types where code = 'damage'),
         (select id from tags where code = 'scoped');
  insert into challenge_rules (challenge_id, action_type_id, required_tag_id)
  select v, (select id from action_types where code = 'damage'),
         (select id from tags where code = 'suppresed');
end $$;

-- ── S10: cañón desde POI con nombre → campamentos piratas distintos ─────────
do $$
...
begin
  ...
  loop
    ...
    values (r, 'from_named_location', 'Saliste de una ubicación con nombre');
  end loop;
end $$;

-- ── report_event: OR + value + same_match suma daño ─────────────────────────
...
begin
  ...
  loop
    ...
    if v_ch.unit = 'distinct_location' then
      ...
      -- OR + value: suma daño de cualquier regla coincidente en la partida
