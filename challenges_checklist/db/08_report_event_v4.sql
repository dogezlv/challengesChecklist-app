-- ============================================================
-- 08_report_event_v4.sql — report_event con:
--   * fases por partida: los desafíos de línea exigen partida activa y la
--     siguiente fase queda bloqueada hasta TERMINAR la partida en la que se
--     completó la anterior (completed_in_match).
--   * implicaciones: visita implícita del lugar del evento + tabla
--     event_implications (dañar desde tirolina = usar tirolina, dañar/matar
--     con un objeto = usarlo, etc.). Profundidad limitada: los eventos
--     sintéticos (visit/use sin lugar) no disparan más implicaciones.
--   * efectos de consumibles (db/06) intactos.
-- ============================================================
...
begin
  ...
  for v_ch in
    ...
    where c.is_completed = false
      ...
      -- fases: bloqueada si la anterior está incompleta O se completó en la
      -- partida aún activa (se desbloquea al terminar la partida)
      and (
    ...
    group by c.id
  loop
    -- los desafíos de partida Y los de fases exigen partida activa
  ...
  -- visita implícita: cualquier acción hecha EN un lugar cuenta como visitarlo
  if p_action_code <> 'visit' and p_location_id is not null then
    v_sub := public.report_event(
      'visit', 1, null, null, null, null, p_location_id, '{}'
    );
    v_updated := v_updated || coalesce(v_sub->'updated', '[]'::jsonb);
    v_skipped := v_skipped || coalesce(v_sub->'skipped', '[]'::jsonb);
  end if;

  -- implicaciones declaradas (p. ej. dañar desde una tirolina = usarla)
  for v_imp in
