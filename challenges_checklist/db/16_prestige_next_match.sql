-- ============================================================
-- 16_prestige_next_match.sql -- El prestigio NO se desbloquea en la MISMA
-- partida en que se completa la semana, sino a partir de la siguiente. Se usa
-- la columna challenges.completed_in_match (la setea el trigger
-- sync_challenge_completion al completar durante una partida y la limpia
-- end_active_match()). Si algun normal de la semana sigue con
-- completed_in_match no nulo => se completo en la partida aun activa => el
-- prestigio sigue bloqueado.
-- ============================================================

create or replace function public.prestige_week_blocked(p_week_id uuid)
returns boolean
language sql
security definer
...
set search_path to 'public'
as $$
  select coalesce(c.is_prestige, false) and public.prestige_week_blocked(c.week_id)
  from challenges c
  where c.id = p_challenge_id;
$$;

-- report_event: el guard del prestigio ahora tambien bloquea si la semana
-- se completo en la partida aun activa (completed_in_match no nulo)
CREATE OR REPLACE FUNCTION public.report_event(p_action_code text, p_amount bigint DEFAULT 1, p_used_object_id uuid DEFAULT NULL::uuid, p_used_tag_id uuid DEFAULT NULL::uuid, p_target_object_id uuid DEFAULT NULL::uuid, p_target_tag_id uuid DEFAULT NULL::uuid, p_location_id uuid DEFAULT NULL::uuid, p_conditions text[] DEFAULT '{}'::text[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
