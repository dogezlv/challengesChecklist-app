-- ============================================================
-- 32_land_mission_rules.sql
--   * S6: aterrizar en 1 de 5 elevaciones + ganar (grupos OR)
--   * S9: aterrizar en POI con nombre tras 3 respiraderos (land + condición)
-- ============================================================

-- ── S6 prestigio: elevación alta + victoria ───────────────────────────────────
do $$
declare
  v uuid;
  r uuid;
...
begin
  ...
  where s.code = 'season_8' and w.week_number = 6 and c.is_prestige
    and c.description ilike '%elevaciones más altas%autobús%'
  limit 1;
  ...
  loop
    grp := grp + 1;
    insert into challenge_rules (challenge_id, action_type_id, location_id, rule_group)
    values (
      v,
      (select id from action_types where code = 'land'),
      (select id from locations where code = loc),
      grp
    )
    returning id into r;
    ...
    insert into challenge_rules (challenge_id, action_type_id, rule_group)
    values (v, (select id from action_types where code = 'misc'), grp)
    returning id into r;
    insert into rule_conditions (challenge_rule_id, condition_key, condition_value)
...
-- ── S9 prestigio: POI con nombre tras 3 respiraderos ──────────────────────────
do $$
...
begin
  ...
  join challenge_weeks w on w.id = c.week_id
  join seasons s on s.id = w.season_id
  where s.code = 'season_8' and w.week_number = 9 and c.is_prestige
    and c.description ilike '%respiraderos volcánicos%diferentes%'
  limit 1;
  if v is null then return; end if;
  ...
  returning id into r;
  insert into rule_conditions (challenge_rule_id, condition_key, condition_value)
  values (r, 'named_landing_after_3_vents', '🌋 Tras 3 respiraderos distintos sin aterrizar');
end $$;

-- Validar POI con nombre en aterrizajes con esa condición
