-- 37: prestigio S9 texto reanimación; S10 materiales 500 c/u; engine AND+value same_match

-- ── S9: eliminar oponente reanimado por furgoneta ─────────────────────────────
update challenges c
set description = 'Elimina a un oponente después de ser reanimado por una furgoneta de reinicio'
...
where c.week_id = w.id
  ...
  and c.description in (
    'Elimina a un oponente reanimado',
    'Elimina a un oponente después de ser reanimado por una furgoneta de reinicio'
  );

update rule_conditions rc
set condition_value = '🔁 Reanimado por furgoneta de reinicio'
...
where rc.challenge_rule_id = cr.id
  ...
  and w.week_number = 9
  and c.is_prestige = true
  and c.description = 'Elimina a un oponente después de ser reanimado por una furgoneta de reinicio';

-- ── S10: 500 de madera, piedra y metal en la misma partida ───────────────────
do $$
declare
  v_id uuid;
  v_harvest uuid;
begin
  ...
  where s.code = 'season_8'
    ...
    and c.is_prestige = true
    and c.description ilike 'Recolecta 500 de %'
  limit 1;

  if v_id is null then return; end if;

  select id into v_harvest from action_types where code = 'harvest';
  ...
  where challenge_rule_id in (select id from challenge_rules where challenge_id = v_id);
  ...
  values
    (v_id, v_harvest, (select id from game_objects where code = 'wood')),
    (v_id, v_harvest, (select id from game_objects where code = 'stone')),
    (v_id, v_harvest, (select id from game_objects where code = 'metal'));
end $$;

-- ── report_event: AND + value + same_match (500 por material) ────────────────
  ...
  loop
    ...
    if v_ch.unit = 'distinct_location' then
      ...
      if v_ch.rules_operator = 'or' and v_ch.unit = 'value' and v_ch.total_rules > 1 then
        ...
        select coalesce(min(coalesce(mrp.amount, 0)), 0),
               coalesce(bool_and(coalesce(mrp.amount, 0) >= v_ch.target_value), false)
